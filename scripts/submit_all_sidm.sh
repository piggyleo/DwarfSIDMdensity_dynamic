...
usage() {
  cat <<'EOF'
Usage: scripts/submit_all_sidm.sh [options] [START_TASK [END_TASK]]
...
for batch_start in $(seq "$start_task" 2 "$end_task"); do
  ...
  echo "Submitting SIDM fit-and-plot batch: $task_spec"
  if submission="$(
    sbatch \
      --parsable \
      --wait \
    ...
    job_id="${submission%%;*}"
    echo "Completed SIDM batch $batch_start-$batch_end: $job_id"
  else
    status=$?
    echo "SIDM batch $batch_start-$batch_end failed with sbatch status $status." >&2
    echo "No later batches were submitted." >&2
    exit "$status"
  fi
done

echo "All SIDM fit-and-plot batches completed for tasks $start_task-$end_task."
